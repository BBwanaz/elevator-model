/*
	...
	Description:	Elevator 1 Process: updates statuses of elevator 1 object
*/
...
int main(int argc, char* argv[])
{
	...
	CMailbox e1Mailbox;									// Mailbox to receive commands from the dispatcher
	UINT Message = 0;									// Command received from the dispatcher
	Elevator e1("Elevator1");							// Object for elevator 1
	CCondition* Door[10];								// Condition for the doors on each floor
	CSemaphore* eL1 = new CSemaphore("EMutex1", 4, 4);	// Counting sempahore for number of passengers
	...
	for (int i = 0; i < 10; i++) {
		sprintf(buffer, "EDoor1%i", i);
		Door[i] = new CCondition(buffer);
	}

	e1.Update_Status(Message, currentFloor, currentRow, door, dir, num_pass);	// Updates the datapool for elevator 1

	while (true) {

		// Update elevator status when there is a new message in the Mailbox
		if (e1Mailbox.TestForMessage()) {
			Message = e1Mailbox.GetMessage();
		...
		// Go to requested floors
			...
			if (currentRow == destinationRow) {
				...
				door = CLOSED;
				num_pass = 4 - eL1->Read();													// Semaphores count downwards
